...
int main(int argc, char *argv[]){
	...
	// waitForDCAF();
    if(createSharedMemory(DCAF_VERSION, &p)) return -1;
    ...
    if(createSharedMemory(STANDARD_ENGINE_SYNC, &p)) return -1;
    ...
    if(createSharedMemory(STANDARD_ENGINE_DOUBLE_INPUT, &p)) return -1;
    ...
    if(createSharedMemory(STANDARD_ENGINE_DOUBLE_OUTPUT, &p)) return -1;
    ...
    if(createSharedMemory(STANDARD_ENGINE_BOOLEAN_INPUT, &p)) return -1;
	...
	if(createSharedMemory(STANDARD_ENGINE_BOOLEAN_OUTPUT, &p)) return -1;
...
//		err = sem_wait(&(*dcaf_sync).start_sem);
		// err = sem_wait(&dcaf_sync->start_sem);
//         err = sem_getvalue(&dcaf_sync->start_sem, &sem_value);
// 		if(err)
// 			break;
//         printf("semaphore value %d", sem_value);
//         // printf("\n");
// 		if(sem_value == 0)
// 			err = sem_post(&dcaf_sync->start_sem);
...
// 			break;
		...
		if(i > 3){
			...
			(*analogInput).mod2_ai0 = voltage;
			(*analogInput).mod2_ai1 = voltage;
			(*analogInput).mod2_ai2 = voltage;
			(*analogInput).mod2_ai3 = voltage;

			(*digitalInput).mod4_di0 = boolean;
			(*digitalInput).mod4_di1 = boolean;
			(*digitalInput).mod4_di2 = boolean;
			(*digitalInput).mod4_di3 = boolean;
		}
